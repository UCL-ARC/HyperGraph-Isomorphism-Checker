...
- Global Interface Colouring: nodes on the global interface are uniquely identified and therefore can be uniquely coloured immediately. The first step in colouring is therefore to iterate over the global interface list (defined as the concatenation of the global inputs followed by global outputs). A new colour (starting at 0 and incrementing each time) is given to each node in this list that is not already coloured (since a node may appear in this concatenated list more than once).
    - In the general case a node can appear in the input list or output list multiple times; in the monogamous case a node can appear in this concatenated list up to twice twice since it can appear in the input list and output list, but every node in the input list should be unique and every node in the output list should be unique.
...
- In the general case the global interface colouring is a form of reduction; it can be performed in $O(N)$ serial time and $O(\log N)$ in a PRAM-CREW model. In the monogamous case the global interface colouring is $O(1)$ in a PRAM-CREW model since every node in the global inputs can be coloured in parallel, and every node in the global outputs can be coloured in parallel.
