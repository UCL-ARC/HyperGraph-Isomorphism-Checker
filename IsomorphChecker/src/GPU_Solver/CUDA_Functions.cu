...
void CreateGraphBinsGPU()
{
    ...
    /* Now we compare Node signatures Label, IO, numNexts, numPrevs */
	    ...
	    for (int gIndex = 0; gIndex < 2; gIndex++)
	    {
	        ...
	        cudaMemcpy(d_temp_numPrevs, dram_Node_edgePrevsNum[gIndex],    numNodes[gIndex]* sizeof(uint), cudaMemcpyDeviceToDevice);
...
//    delete []h_target_hashes;
  ...
  /*=========================================================================*/
	...
	cudaMalloc((void**)&d_original_indices_srcA, numEdges[0] * sizeof(uint));
	cudaMalloc((void**)&d_original_indices_tgtA, numEdges[0] * sizeof(uint));
	...
	thrust::sequence(d_ptr_indices_src_A, d_ptr_indices_src_A + numEdges[0]);
	thrust::sequence(d_ptr_indices_tgt_A, d_ptr_indices_tgt_A + numEdges[0]);


	cudaMalloc((void**)&d_original_indices_srcB, numEdges[1] * sizeof(uint));
	cudaMalloc((void**)&d_original_indices_tgtB, numEdges[1] * sizeof(uint));
	...
	thrust::sequence(d_ptr_indices_src_B, d_ptr_indices_src_B + numEdges[1]);
	thrust::sequence(d_ptr_indices_tgt_B, d_ptr_indices_tgt_B + numEdges[1]);
