...
def test_edge_signatures(mock_edges):
    ...
    assert signature.sources[0].label == "X"
    assert signature.sources[1].label == "Y"
...
def test_node_reference_validation(mock_nodes):
    ...
    assert any("reference nodes not in hypergraph" in error for error in errors)
    assert hypergraph.is_valid() is False
